...
fail=0; n=0
for f in $(ls agent/*_rust_parity_test.py pet/*_rust_parity_test.py 2>/dev/null) agent/rust_unicode_fuzz_parity_test.py agent/rust_python_text_semantics_parity_test.py; do
  m="$(echo "${f%.py}" | tr '/' '.')"
  n=$((n+1))
