...
8. До 1000 слов — это ПОТОЛОК, а не обязательная длина: пиши ровно
   настолько подробно, насколько того требует вопрос и доступные
   наблюдения; короткий точный ответ так же уместен, как и длинный
...
НЕ пиши отдельный раздел "СТАТУС" или свою оценку
(WEAKLY/PARTIALLY/STRONGLY_SUPPORTED и т.п.) — итоговый Trust
вычисляется пайплайном отдельно, на основе реальной проверки evidence,
ПОСЛЕ того как этот текст уже сгенерирован, и показывается пользователю
в подписи к ответу. Твоя самооценка статуса здесь была бы отдельным,
не связанным с реальной проверкой числом — не дублируй и не предвосхищай
итоговый Trust.
...
- До 1000 слов — это ПОТОЛОК, а не обязательная длина: пиши столько,
  сколько нужно по существу; короче тоже нормально
...
def synthesize(
    ...
    try:
        if context:
            ...
            raw_answer = _call(compose_prompt, max_tokens=MAX_TOKENS_ANALYST, temp=TEMP_ANALYST)
        else:
            raw_answer = _call(
