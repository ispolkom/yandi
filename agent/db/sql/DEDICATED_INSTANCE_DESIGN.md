...
| Root filesystem | `/dev/<disk>`, 218G total, currently 126G free (40% used) — see `DISK_CAPACITY_REPORT.md` for the full, separately-flagged volatility note |
...
Chosen over `/mnt/<volume>` (see `DISK_CAPACITY_REPORT.md`'s candidate-
location analysis) because `/dev/<disk>` follows Linux FHS convention for
