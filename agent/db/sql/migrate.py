...
SCHEMA_VERSION_DESCRIPTION = "v17: personal_fact + personal_fact_event (provenance-backed append-only ledger of what the person reported about themselves). v16: interaction_turn (immutable per-person source history of chat turns, keyed by the client-minted turn id). v15: commitment + commitment_event + causal_event (immutable promise ledger + causal-event idempotency ledger). v14: knowledge_query_archive (\"точка ноль\" — agent/db/manager.py's sqlite KnowledgeDB query-log/moderation-queue retired)"
