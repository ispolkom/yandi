...
def main() -> int:
    ...
    led = [
        ("record_interaction_turn", dict(user_id="u1", source_turn_id="t1", turn_id_origin="client", user_text="Привет! Я обещаю прийти завтра.", assistant_text="Хорошо", model="m", adapter="llama_cpp", recalled_turn_ids=["t0"], created_at=TS[0])),
        ("record_interaction_turn", dict(user_id="u1", source_turn_id="t1", turn_id_origin="client", user_text="ДУБЛЬ", assistant_text=None, created_at=TS[1])),
        ("record_interaction_turn", dict(user_id="u1", source_turn_id="t2", turn_id_origin="server", user_text="я" * 21000, assistant_text="а" * 20500, recalled_turn_ids=[], created_at=TS[2])),
        ("record_interaction_turn", dict(user_id="u1", source_turn_id="t3", turn_id_origin="client", user_text="yp1:как запечатанное", assistant_text=None, created_at=TS[3])),
        ("record_interaction_turn", dict(user_id="u2", source_turn_id="t1", turn_id_origin="client", user_text="другой человек", assistant_text=None, created_at=TS[3])),
        ("get_interaction_turn_text", dict(user_id="u1", source_turn_id="t1")),
        ("get_interaction_turn_text", dict(user_id="u1", source_turn_id="t3")),
        ("get_interaction_turn_text", dict(user_id="u1", source_turn_id="zz")),
        ("claim_causal_event", dict(user_id="u1", source_turn_id="t1", event_type="promise", span_start=5, span_end=20, created_at=TS[0])),
        ("claim_causal_event", dict(user_id="u1", source_turn_id="t1", event_type="promise", created_at=TS[1])),
        ("claim_causal_event", dict(user_id="u1", source_turn_id="t1", event_type="apology", created_at=TS[1])),
        ("list_recent_interaction_turns", dict(user_id="u1")),
        ("list_recent_interaction_turns", dict(user_id="u1", limit=1)),
        ("insert_personal_fact", dict(fact_id="f1", user_id="u1", fact_class="identity", statement="Меня зовут Аня", polarity="affirm", temporality="current", evidence="Я Аня", span_start=0, span_end=5, source_turn_id="t1", created_at=TS[0])),
        ("insert_personal_fact", dict(fact_id="f2", user_id="u1", fact_class="identity", statement="Живу в Риге", polarity="affirm", temporality="current", evidence="в Риге", span_start=None, span_end=None, source_turn_id="t3", created_at=TS[1])),
        ("insert_personal_fact", dict(fact_id="f3", user_id="u1", fact_class="x", statement="s", polarity="p", temporality="t", evidence="e", span_start=1, span_end=2, source_turn_id="нет-такого", created_at=TS[1])),
        ("insert_personal_fact_event", dict(fact_id="f1", user_id="u1", event_type="restated", by_fact_id=None, evidence="снова Аня", span_start=1, span_end=3, source_turn_id="t3", created_at=TS[2])),
        ("insert_personal_fact_event", dict(fact_id="f1", user_id="u1", event_type="superseded", by_fact_id="f2", evidence="иначе", span_start=None, span_end=None, source_turn_id="t3", created_at=TS[3])),
        ("list_personal_facts", dict(user_id="u1")),
        ("list_personal_facts", dict(user_id="u1", limit=1)),
        ("list_personal_fact_events", dict(user_id="u1")),
        ("record_commitment", dict(commitment_id="c1", user_id="u1", kind="promise", text="Приду завтра " + "х" * 600, evidence="обещаю", due_at=TS[5], created_at=TS[0], source_turn_id="t1")),
        ("record_commitment", dict(commitment_id="c2", user_id="u1", kind="promise", text="Позвоню", evidence="позвоню", due_at=1767225600, created_at=TS[1])),
        ("record_commitment", dict(commitment_id="c3", user_id="u2", kind="k", text="t", evidence="e", created_at=TS[1])),
        ("get_commitment", dict(commitment_id="c1")),
        ("get_commitment", dict(commitment_id="zz")),
        ("list_commitments", dict(user_id="u1")),
        ("record_commitment_event", dict(commitment_id="c1", user_id="u1", event_type="fulfilled", source="user", evidence="пришёл", created_at=TS[2], source_turn_id="t3", span_start=0, span_end=5)),
        ("record_commitment_event", dict(commitment_id="c1", user_id="u1", event_type="fulfilled", source="user", evidence="дубль", created_at=TS[3])),
        ("record_commitment_event", dict(commitment_id="c1", user_id="u1", event_type="broken", source="system", evidence=None, created_at=TS[3])),
        ("record_commitment_event", dict(commitment_id="c2", user_id="u1", event_type="fulfilled", source="user", evidence="", created_at=TS[3])),
        ("list_commitment_events", dict(user_id="u1")),
        ("count_commitment_events", dict(user_id="u1", event_type="fulfilled", source="user")),
        ("count_commitment_events", dict(user_id="u1", event_type="fulfilled", source="nobody")),
        ("create_disagreement", dict(disagreement_id="dg1", topic="t", old_position="o", challenge="c", analysis="a", new_position="n", confidence_before=0.8, confidence_after=0.45, resolved=False, related_belief_id="b1", created_at=TS[1])),
        ("create_disagreement", dict(disagreement_id="dg2", topic="t2", old_position="o", challenge="c", analysis="a", new_position="n", confidence_before=0.1, confidence_after=0.2, created_at=TS[0])),
        ("list_disagreements", dict()),
    ]
    scenario("J1 личный журнал (защита выключена)", led)
    misc2 = [
        ("get_trait_graph", dict()),
        ("get_or_create_trait_graph", dict(nodes={"a": 0.5}, edges={"a->b": 0.3}, updated_at=TS[0])),
        ("get_or_create_trait_graph", dict(nodes={}, edges={}, updated_at=TS[5])),
        ("update_trait_graph", dict(nodes={"a": 0.6, "б": 1}, updated_at=TS[1])),
        ("update_trait_graph", dict(updated_at=TS[2])),
        ("get_trait_graph", dict()),
        ("record_trait_change", dict(node="a", new_value=0.61, source="chat", created_at=TS[0])),
        ("record_trait_change", dict(node="a", new_value=0.62, created_at=TS[1])),
        ("record_trait_change", dict(node="b", new_value=0.1, created_at=TS[2])),
        ("list_trait_changes", dict()),
        ("list_trait_changes", dict(limit=2)),
        ("list_trait_changes", dict(since=TS[1])),
        ("record_trait_edge_change", dict(source_node="a", target_node="b", old_weight=0.1, new_weight=0.2, created_at=TS[0])),
        ("get_or_seed_internal_questions", dict(default_questions=["Кто я?", "Зачем?"], created_at=TS[0])),
        ("get_or_seed_internal_questions", dict(default_questions=["Ещё", "Ещё2"], created_at=TS[1])),
        ("record_internal_question_answer", dict(question_id=1, answer_text="ответ 1", created_at=TS[2])),
        ("record_internal_question_answer", dict(question_id=1, answer_text="ответ 2", created_at=TS[1])),
        ("record_internal_question_answer", dict(question_id=99, answer_text="сирота", created_at=TS[1])),
        ("list_internal_questions", dict()),
        ("get_self_reflection_profile", dict()),
        ("get_or_create_self_reflection_profile", dict(desires=["a"], fears=[], likes=["л"], dislikes=[], limitations=["x"], updated_at=TS[0])),
        ("get_or_create_self_reflection_profile", dict(desires=[], fears=[], likes=[], dislikes=[], limitations=[], updated_at=TS[3])),
        ("increment_self_reflection_count", dict(updated_at=TS[4])),
        ("increment_self_reflection_count", dict(updated_at=TS[5])),
        ("get_self_reflection_profile", dict()),
        ("upsert_social_knowledge", dict(speech_act="thanks", topic="общее", description="d", typical_reactions=["r"], cultural_context="c", boundaries=[], recommended_approach="ra", examples=["e1"], created_at=TS[0], updated_at=TS[0])),
        ("upsert_social_knowledge", dict(speech_act="thanks", topic="общее", description="d2", typical_reactions=[], cultural_context="c2", boundaries=["b"], recommended_approach="ra2", examples=[], source="me", confidence=0.9, created_at=TS[5], updated_at=TS[5])),
        ("get_social_knowledge", dict(speech_act="thanks", topic="общее")),
        ("get_social_knowledge", dict(speech_act="thanks", topic="нет")),
    ]
    scenario("J2 граф черт, внутренние вопросы, профиль, социальные знания", misc2)
    kqa = [
        ("record_knowledge_query", dict(entry_id="q1", query="Сколько планет?", answer="8", tag="science:astronomy", category="science", trust_level="UNVERIFIED", confidence=0.4, sources=["s1"], node_id="n1", meta={"k": 1}, created_at=TS[0], updated_at=TS[0])),
        ("record_knowledge_query", dict(entry_id="q1", query="Сколько планет?", answer="9", tag="science:astronomy", category="science", trust_level="UNVERIFIED", confidence=0.5, sources=[], meta=None, created_at=TS[1], updated_at=TS[1])),
        ("get_knowledge_query", dict(entry_id="q1")),
        ("set_knowledge_query_verified", dict(entry_id="q1", updated_at=TS[2])),
        ("record_knowledge_query", dict(entry_id="q1", query="x", answer="НЕ ДОЛЖНО ПЕРЕЗАПИСАТЬ", tag="science:astronomy", category="science", trust_level="UNVERIFIED", confidence=0.1, created_at=TS[3], updated_at=TS[3])),
        ("get_knowledge_query", dict(entry_id="q1")),
        ("record_knowledge_query", dict(entry_id="q1", query="x", answer="повторная проверка", tag="science:astronomy", category="science", trust_level="VERIFIED", confidence=0.99, sources=["s9"], created_at=TS[4], updated_at=TS[4])),
        ("get_knowledge_query", dict(entry_id="q1")),
        ("record_knowledge_query", dict(entry_id="q2", query="Другое", answer="A", tag="history", category="history", created_at=TS[1], updated_at=TS[1])),
        ("record_knowledge_query", dict(entry_id="q3", query="Ещё", answer="B", tag="science:physics", category="science", confidence=0.8, created_at=TS[2], updated_at=TS[2])),
        ("update_knowledge_query_answer", dict(entry_id="q2", answer="новый ответ", updated_at=TS[5])),
        ("update_knowledge_query_answer", dict(entry_id="zz", answer="x", updated_at=TS[5])),
        ("set_knowledge_query_verified", dict(entry_id="zz", updated_at=TS[5])),
        ("list_unverified_knowledge_queries", dict()),
        ("list_knowledge_queries_by_tag", dict(tag="science")),
        ("list_knowledge_queries_by_tag", dict(tag="science", min_confidence=0.9)),
        ("list_knowledge_queries_by_tag", dict(tag="history", limit=1)),
        ("list_knowledge_query_tags", dict()),
        ("knowledge_query_archive_stats", dict()),
        ("delete_knowledge_query", dict(entry_id="q3")),
        ("delete_knowledge_query", dict(entry_id="zz")),
    ]
    scenario("J3 архив запросов знаний", kqa)
    scenario("J4 INSERT IGNORE и внешние ключи", [
        ("link_family_member", dict(family_id="нет-семьи", claim_id="c1", linked_at=TS[0])),
        ("record_commitment_event", dict(commitment_id="нет-обязательства", user_id="u1", event_type="fulfilled", source="user", created_at=TS[0])),
