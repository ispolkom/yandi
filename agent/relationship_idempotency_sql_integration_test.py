...
def main() -> int:
    ...
    check("S: the current schema version is recorded", scalar(root, "SELECT MAX(version) FROM schema_migrations") == schema.SCHEMA_VERSION == 20)
    ...
    with root.cursor() as cur:
        ...
        cur.execute("DELETE FROM schema_migrations WHERE version IN (15, 16, 17, 18, 19, 20)")
