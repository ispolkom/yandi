...
В реальном project venv (`/home/user/venv`, тот же, которым пользуется
...
и прямым HTTP-пробом через `/home/user/venv/bin/python3` с точно теми
...
адрес `<proxy-host>:<port>`) — но это не задевает production-код: он
...
через `/home/user/venv/bin/python3` (`got 0.5500500202178955`, побитово
...
Через `/home/user/venv/bin/python3` (реальный project venv, тот же,
...
```
$ /home/user/venv/bin/python3 -m py_compile \
    agent/source_quality.py \
    agent/evidence_eligibility_regression_test.py
# без вывода — OK

$ /home/user/venv/bin/python3 -m agent.evidence_eligibility_regression_test
...
...
что в проекте есть полноценный venv `/home/user/venv` со всеми
...
`/home/user/venv/bin/python3`, а не системный интерпретатор — это
