...
still does not encrypt the SQL/personal storage (P1c). See *What still goes around the node*.
...
2. Storage encryption is not bound to the Node-derived key (P1c): `lock` is an execution gate for this Core process, not yet a data decryption boundary.
