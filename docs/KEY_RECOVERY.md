# Key recovery (P1c-1) — the root key, the device, and the master password you create
...
## First run: you create the passwords yourself (nothing is generated)

On a machine with no `auth.json`, the node starts **only a setup page** (`http://127.0.0.1:<port>`, loopback only, nothing else is served) and waits. You type:

1. a **login password** (at least 8 characters) and repeat it — this opens the web UI;
2. a **master password** (at least 12 characters, different from the login password) and repeat it — this is the **recovery secret**: write it on paper.

Only after both are typed twice and saved does the node create the root key, the device key and the identity, and continue starting. The master password is stretched
with Argon2id; it is not stored. Nothing is generated for you and nothing is shown for you to copy. Later, on a new machine or after losing the device key, the same
master password brings the same node id back (`yandi-keys recover`), and the login page's **"Забыли пароль?"** accepts it to set a new login password.

## The generated recovery code (optional)

If you would rather not choose a secret, `yandi-keys migrate --generate-code` makes one for you. It is optional; the rule for the first-run setup and for `migrate`
is that **the person creates the secret**.

It is shown once as `XXXX-XXXX-XXXX-XXXX-XXXX-XXXX-CCCC` (120 random bits, no look-alike letters; the last group is a check). You write it down, then **type it back**
before anything is saved; case, spaces and dashes do not matter; a typo is reported as a typo ("does not pass its check"), not as "wrong". It is used exactly like a chosen
password (Argon2id over its canonical form), so no file format differs. It is **not** the web login password and cannot be recovered: write it on paper, keep it away from the key files.
...
# an existing (legacy) directory: you type YOUR master password twice (typed with no echo; --show shows it), then it migrates (stop the node first)
./target/release/yandi-keys migrate
./target/release/yandi-keys migrate --generate-code     # or: let the system make a code, shown once, that you type back

# already migrated and the recovery secret is lost or does not fit: replace it (the DEVICE opens the key; node id, identity and chats are untouched)
...
./target/release/yandi-keys recover --dir ~/recovery_test/.yandi_keys     # type the master password / code (hidden as you type; --show shows it)
rm -rf ~/recovery_test
```
...
~/yandi/node/target/release/yandi-keys recover        # asks for the master password (or recovery code)
...
(`mv ~/.yandi_keys ~/.yandi_keys.old`, never delete it), stop any running node, start the node, open the setup page and type the new login password and the new master
password (each twice). The node then has a recoverable key from its first second; `yandi-keys migrate` is not needed. Warnings: the node id changes, chats made under
the old identity do not open, and paired devices must be paired again.
