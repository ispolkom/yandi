...
def main() -> int:
    ...
    try:
        ...
        # ---- Ollama ИСКЛЮЧЁН из родной версии (решение владельца 2026-09-25): моста ollama_generate больше нет ----
        n += 1
        try:
            rs("ollama_generate", messages=MSGS[0], model="m", base_url=srv.url)
            check("H0 ollama_generate отсутствует в родном мосте", False, "функция ответила")
        except ValueError as e:
            check("H0 ollama_generate отсутствует в родном мосте", "неизвестная функция" in str(e), str(e))
