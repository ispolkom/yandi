...
**Срез 41 — Ollama: ИСКЛЮЧЁН из родной версии (решение владельца, 2026-09-25: «даже как фолбэк — только свои инструменты»).** Модуль `ollama.rs` был написан и проверен (13/13 мутантов), затем УДАЛЁН вместе с мостовой функцией; у родного шлюза нет ни одного пути к `/api/`. Python-версия пока не тронута (её откат на Ollama остаётся оракулом и для встроенных эмбеддингов нужен собственный движок).
**Срез 45 — собственный локальный движок узла `server_engine.rs`:** замена Python-библиотеки `llama_cpp` — узел САМ запускает и присматривает за процессом `llama-server` (llama.cpp) и говорит с ним по OpenAI-протоколу на localhost (тот же `remote.rs`). Ленивый запуск при первом обращении (свободный порт, `-m/-c/-ngl`, `--embedding` для эмбеддингов — отдельный экземпляр), ожидание `/health` (503 «модель грузится» терпится), упавший процесс поднимается заново при следующем вызове, процесс, не ставший готовым за срок, убивается, при выходе узла все дочерние процессы убиваются. `llama-server` ВШИВАЕТСЯ в бинарник узла (`build.rs`, при сборке `YANDI_EMBED_LLAMA_SERVER=dist/llama-server`; собирает его `scripts/build-llama-server.sh`), при первом запуске сам распаковывается в каталог данных узла (`engine_binary.rs`: вшитый → рядом с узлом → каталог данных → PATH; `YANDI_LLAMA_SERVER` — только переопределение для разработчика). Пользователь ничего не ставит и не указывает. Семантика Python-движка сохранена: «GGUF-файл не найден», json-режим только при `response_format == "json"` (схема не передаётся), `repeat_last_n` отбрасывается, явные `temperature`/`max_tokens` сильнее `extra_options`, один запрос за раз. Тест `tests/server_engine_test.rs` (7 сценариев, ЗАПУСКАТЬ `-- --test-threads=1`: подделка `tests/fake_llama_server.py` с теми же аргументами и путями; проверяются командная строка, перезапуск, убийство при Drop/тайм-ауте, сквозной путь через `Gateway` с «сетью», которая паникует); ~25 мутантов, все наблюдаемые пойманы (3 сначала выжили — закрыты: `-ngl 0`, строка ≠ "json", приоритет `max_tokens`; 1 эквивалентен на Linux). Реального `llama-server` в песочнице нет. Проверка на настоящей модели — БЕЗ аргументов: `cargo run --release --no-default-features --example engine_check` (читает настройки узла, т.е. модель, выбранную пользователем в вебе, и идёт путём Gateway → настройка → ServerEngine).
**Срез 46 — родной «мост интеллекта» (`intelligence.rs`) и ПОДКЛЮЧЕНИЕ движка к узлу:** то, что делал Python-мост `llm_gateway/intelligence_bridge.py` (межнодовые и локальные AI-запросы → один алиас `yandi:peer-default`, поля model/base_url из запроса игнорируются, наружу только `backend_error`), теперь работает внутри процесса узла. Узел (`node/src/ai_rpc/intelligence_bridge.rs`) выбирает мост так: `YANDI_INTELLIGENCE_ENGINE=native|python`; по умолчанию — родной, если в бинарник вшит движок (`YANDI_EMBED_LLAMA_SERVER`), иначе прежний Python-мост. Один общий `ServerEngine` на процесс (две копии модели в памяти не нужны). Тест `llm_gateway/native_intelligence_parity_test.py` (73 проверки против Python-обработчика: валидация входа, форма и порядок ключей ответа, запрос, дошедший до сервера); 17 мутантов, все пойманы (3 сначала выжили: байты против символов в лимите 32768, bool в temperature, роль `tool`). Известные безвредные отличия: температура 0/1 у Python уходит целым, у Rust `0.0/1.0`; bool в `max_tokens`/`temperature` Rust приводит к числу; для не настроенного алиаса Python откатывается на Ollama, родной — честная ошибка без запросов. Сквозной тест узла: `node/tests/native_intelligence_link.rs` (`-- --test-threads=1`).
...
явный выбор владельца узла (secure_store) сильнее любого автоматического отката — успех или честная ошибка ИМЕННО этого backend'а, никакого тихого перехода на другой backend (Ollama в родной версии не существует вовсе); флаг `local_enabled` управляет ТОЛЬКО автоматической
...
`UnavailableEngine`, как `Llama is None` в оригинале, либо `ServerEngine` — срез 45). Тест `llm_gateway/native_client_parity_test.py` (243 проверки): один общий HTTP-сервер, настройки узла подменены ОДНОЙ картой и для Python, и для Rust;
...
явная настройка remote openai/anthropic, плохой протокол/backend, запись без поля, llamacpp с несуществующим файлом, исключение хранилища, семантика по двум контрактам, эмбеддинги (настроено/битые батчи/отказ Anthropic). Блок «NO*» — проверки ТОЛЬКО родной версии: не настроено → честная ошибка «не настроен ни один backend» БЕЗ единого запроса; реестр включён, движок недоступен → ошибка движка без отката; явный base_url → OpenAI-протокол; за весь прогон ни одного запроса на `/api/`.
...
Дальше по плану: `config`/`setup` (тонкие обёртки), слой БД (решение владельца), эпистемический конвейер, веб-сервер на axum.

## Встроенное хранилище состояния `rustlib/yandi_state` (срез 44, 2026-09-25)

Замена внешнему Redis в едином бинарнике (кроссплатформенно: без сервера, без установки; на Windows/macOS Redis «из коробки» нет). Реализована ровно та часть Redis, которую код YANDI реально использует:
строки (`GET`/`SET` со всеми опциями `EX/PX/EXAT/PXAT/NX/XX/GET/KEEPTTL`, `SETEX/PSETEX/SETNX/GETSET/MGET/MSET/APPEND/STRLEN/INCR*/DECR*`), сроки жизни (`EXPIRE/PEXPIRE` с `NX/XX/GT/LT`, `TTL/PTTL/PERSIST`),
`DEL/EXISTS/TYPE/RENAME/DBSIZE/FLUSHDB/KEYS/SCAN`, списки (`LPUSH/RPUSH/LPUSHX/RPUSHX/LPOP/RPOP` со счётчиком, `LLEN/LRANGE/LTRIM/LINDEX/LSET/LREM/LINSERT`), множества (`SADD/SREM/SCARD/SISMEMBER/SMEMBERS`),
`PING`, `PUBLISH/SUBSCRIBE/PSUBSCRIBE` (число получателей как в Redis: канальные + шаблонные). Срок хранится АБСОЛЮТНЫМ в миллисекундах, как в Redis (важно для `GT/LT` и округления `TTL`); шаблоны ключей — побайтовый порт `stringmatchlen`.
Один процесс (хранилище живёт внутри процесса ядра; общий доступ нескольких процессов — через API ядра). Тест `agent/state_native_parity_test.py` поднимает ПРИВАТНЫЙ настоящий `redis-server` (свободный порт, без сохранения на диск; живой Redis владельца НЕ затрагивается) и гонит
ему и встроенному хранилищу ОДНИ И ТЕ ЖЕ команды: ~11,4 тыс. сравнений (направленные случаи + 150 случайных последовательностей над малым набором ключей, чтобы команды «сталкивались», + типы-столкновения WRONGTYPE между строкой/списком/множеством) — сравниваются КАЖДЫЙ ответ, тексты ошибок и
ПОРЯДОК проверок аргументов (в Redis у каждой команды свой: где раньше проверяется число, где ключ, где тип — тест это и обнаружил). Плюс подписка/публикация, сроки жизни в реальном времени.
**Мутанты: 63 внесённых, все наблюдаемые пойманы** (9 сначала выжили — не хватало: точной проверки `-1/-2` и округления TTL, `LREM` с дубликатами, `LINSERT` в свежий список, очистки просроченного самими `DBSIZE/KEYS`, длинных аргументов в неизвестной команде; допуск TTL скрывал ошибки — убран);
эквивалентны 5 (проверка длины числа дублирует разбор, строгость `>` в границе миллисекунды, обрезка правой границы диапазона, счёт `LREM` с нулевым счётчиком, очистка подписчика при `Drop` — отправка в закрытый приёмник и так не считается получателем).
Расхождения, не считающиеся ошибкой: `SCAN` отдаёт весь набор за один шаг (курсор 0; у Redis на маленьких таблицах курсор тоже «схлопывается»), порядок `KEYS`/`SMEMBERS` не определён (сравнивается как множество). Тексты ошибок — Redis 7.0.
ПЕСОЧНИЦА: `maturin develop` для этого крейта офлайн не проходит из-за metadata всего рабочего пространства — там сборка вручную `cargo build --release` + копия `.so` (у владельца с сетью — обычный `maturin develop`).
Дальше: подключить хранилище к ядру (вместо `redis.Redis`) через тонкий слой `state store` и перенести сценарии `pet/`/`agent/`, использующие Redis.
