...
`UnavailableEngine`, как `Llama is None` в оригинале — в Rust-варианте это будет supervised-процесс `llama-server` + OpenAI-протокол). Тест `llm_gateway/native_client_parity_test.py` (243 проверки): один общий HTTP-сервер, настройки узла подменены ОДНОЙ картой и для Python, и для Rust;
