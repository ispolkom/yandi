...
**Остаточные ограничения (известны, задокументированы, не устранимы штатными средствами):**
* `\w` / `\b` в regex-паттернах крейта `regex` определяются иначе, чем в Python (например `²`, комбинирующие знаки).
  Точная замена потребовала бы lookaround. Затрагивает только экзотические символы рядом со словами-маркерами.
  (В срезе 15 токенизатор обходит это таблицей `py_word_table.rs`.)
...
* Таблицы Unicode сгенерированы под Python 3.11.2 / Unicode 14.0; при смене версии Python на боевой машине —
  перегенерировать (`python rustlib/gen_py_*_table.py`).
