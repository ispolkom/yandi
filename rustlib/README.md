...
## Путь к единому бинарнику (2026-09-24)

Цель владельца: один скомпилированный бинарник, в который входит всё, и один язык — так проще собирать кроссплатформенное приложение.
Первый шаг сделан: `rustlib/yandi_rs` собирается в ДВУХ режимах —

* **по умолчанию** (`python` включено): нативный модуль для Python (`maturin develop --release`) — как раньше, Python остаётся основным;
* **`--no-default-features`**: ЧИСТАЯ Rust-библиотека без Python и без PyO3 (`cargo test --no-default-features --lib` — 186 тестов).
  Всё, что привязано к объектам Python (`#[pyfunction]`, `Bound<PyAny>`, обёртки `register`, разбор словарей/списков Python), закрыто флагом
  `#[cfg(feature = "python")]`; чистые ядра (`pub fn` над строками/числами) доступны обоим режимам.

Узел (`node/`) подключает библиотеку как обычную зависимость (`yandi_rs = { path = "../rustlib/yandi_rs", default-features = false }`);
`node/tests/native_core_link.rs` доказывает, что перенесённая логика (шлюз shell, шифрование полей, Python-точные `lower`/`casefold`, токенизатор)
работает внутри рабочего пространства узла, без интерпретатора. Поведение узла это не меняет.

Что ещё нужно до настоящего единого бинарника (по убыванию срочности): (1) хранилище состояния вместо Redis, (2) `llm_gateway` на Rust,
...
**Срез 41 — Ollama (`ollama.rs`):** `/api/chat` (`stream:false`, `options`/`format`), возвращается текст И весь сырой ответ; те же 460+ сценариев общего сетевого теста (в т.ч. запрос побайтно, порядок ключей, ошибки статусов, соединение), **13 из 13 внесённых мутантов пойманы** (1 сначала выжил — не было теста ошибки соединения). Адрес НЕ обрезается по слэшу (как в оригинале — путь `//api/chat`).
**Локальный движок (`llamacpp_backend`)** в оригинале — Python-библиотека `llama_cpp` внутри процесса; в Rust-варианте естественная замена — supervised-процесс `llama-server` + тот же OpenAI-протокол (`remote.rs` его уже говорит); решается вместе с упаковкой бинарника, здесь не дублируется.
**Срез 42 — хранилище настроек `secure_store.rs`:** зашифрованное, «вставить-или-стереть, но никогда не изменить» хранилище настроек моделей узла на встроенной SQLite (`rusqlite` с `bundled` — SQLite прямо в бинарнике).
Родная версия ФАЙЛОВО СОВМЕСТИМА с Python: та же схема и триггеры (тексты CREATE совпадают), файл ключа (32 байта, 0600, каталог 0700), файл контрольной точки цепочки рядом с ключом (тот же формат `{"seq": N, "hash": "hex"}`),
шифроблоки (`crypto.rs`), слепой индекс имени, хеш-цепочка журнала. Тест `llm_gateway/native_secure_store_parity_test.py`: Python пишет → Rust читает; Rust пишет с нуля (создаёт ключ сам) → Python читает; смешанная цепочка операций;
подделки (строка в обход журнала / удалена / изменена, запись журнала испорчена, откат базы старой копией, контрольная точка удалена/испорчена/отстаёт/опережает, база стёрта, журнал пуст, ключ удалён, права 0644, ключ не 32 байта,
старая схема, запись с чужим ключом при согласованном журнале) — из ОДНОГО снимка файлов исполняются обе реализации, результат и ТЕКСТ ошибки совпадают, итоговое состояние читается другой реализацией одинаково. 268 сценариев; ~37 внесённых мутантов ВСЕ пойманы (5 сначала выжили — не хватало проверок: маска прав 0640/0660, длина ключа 33/64, seq=1 и режим WAL у базы, созданной Rust, недешифруемая запись при согласованном журнале).
Отличие (не ошибка): открытый текст конфигурации родная версия сериализует компактно (`serde_json`), Python — `json.dumps` с пробелами. Это шаг к единому бинарнику БЕЗ внешнего сервера: SQLite встроена; решение «MySQL или встроенная база» для памяти агента остаётся за владельцем.
**Срез 43 — клиент шлюза `client.rs`:** `resolve_target`, `complete`, `complete_with_meta`, `complete_semantic`, `embed` — единая точка вызова модели и ГЛАВНЫЙ ИНВАРИАНТ сохранён дословно:
явный выбор владельца узла (secure_store) сильнее любого автоматического отката — успех или честная ошибка ИМЕННО этого backend'а, никакого тихого перехода на Ollama/встроенный движок; флаг `local_enabled` управляет ТОЛЬКО автоматической
загрузкой встроенного движка, когда владелец ничего не настраивал; эмбеддинги — отдельная способность с тем же инвариантом. Внешнее подключается через трейты `Transport` (HTTP), `ConfigSource` (secure_store), `LocalEngine` (встроенный движок; пока
`UnavailableEngine`, как `Llama is None` в оригинале — в Rust-варианте это будет supervised-процесс `llama-server` + OpenAI-протокол). Тест `llm_gateway/native_client_parity_test.py` (337 сценариев): один общий HTTP-сервер, настройки узла подменены ОДНОЙ картой и для Python, и для Rust;
сравниваются запросы (сырой путь, заголовки, тело) и результат вместе с трассой попыток (`_llm_gateway_trace`: адаптер, источник, причина выбора, возможности, контракт, класс и текст ошибки) либо ошибка (класс + текст):
явная настройка remote openai/anthropic, плохой протокол/backend, запись без поля, llamacpp с несуществующим файлом, исключение хранилища, НЕ настроено → Ollama, явный base_url, встроенный реестр (движок недоступен → откат на Ollama ТОЛЬКО для встроенной цели), семантика по трём контрактам, эмбеддинги (настроено/нет/битые батчи/отказ Anthropic).
Мутанты: ~55; все наблюдаемые пойманы (7 выживших сначала закрыты новыми проверками: вид адреса в трассе, «висящие» теги размышлений, питоновский strip); 2 эквивалентны в тестовой обвязке (`config_ref` и `n_ctx` по умолчанию не выводятся, пока движок не подключён).
Текст `json.loads`/`resp.json()` в ошибках бэкендов теперь Python-точный (`Expecting value: line 1 column 1 (char 0)`).
Дальше по плану: `config`/`setup` (тонкие обёртки), локальный движок (`llama-server`), затем хранилище состояния, слой БД (решение владельца), конвейер, веб-сервер.
...
| `agent/orchestrator/claims/status.py::evaluate_claim_status_gate` — **ШЛЮЗ СТАТУСОВ** (срез 38): подсчёт статусов утверждений и по результату потолок доверия/уверенности + предупреждение «⚠️ ВАЖНО: …» прямо в ТЕЛЕ ответа (пять взаимоисключающих случаев: нет утверждений / все отклонены / только опровергнутые / есть спорные / verified=0). Работает над объектом `synthesis_result` через getattr/setattr, `log` вызывается в тех же местах, что в оригинале | тот же `yandi_rs/src/claim_status.rs` | тот же `YANDI_CLAIM_STATUS_ENGINE=rust` | Собрано, 40 тыс. случайных сравнений (dataclass/SimpleNamespace/`__slots__`/property/замороженный dataclass/объект без полей; статусы, включая None/`weak`/пустую строку/посторонние типы; все значения trust_level/confidence, включая NaN/inf/bool/строку/Decimal; ответ с уже стоящим «⚠️», без селектора, пустой) + все пять веток × все уровни доверия × ответ с/без пометки, точные тексты предупреждений сверены дословно; при неоднозначности типов (ответ не str, confidence не число, trust_level нехэшируем) Rust ничего не делает и не логирует — исполняется Python; **30 из 32 внесённых мутантов пойманы, 2 эквивалентны** (`rejected >= total` недостижимо; повторная установка уже стоящего WEAKLY_SUPPORTED) (`agent/claim_status_rust_parity_test.py`); в бою по умолчанию ВЫКЛЮЧЕНО |
