...
================================================================================
Дата: 2026-08-27
Скоуп: /home/user/yandi/agent/ ТОЛЬКО.
...
1. **Диагностика была искажена прокси.** В окружении сессии установлены
   ...
   (`<proxy-host>:<port>`), который не может достучаться до loopback-адреса →
...
пока идёт о /home/user/yandi/agent/"). **STOP на этой подзадаче.**
...
Все regression-тесты запускались через `/home/user/venv/bin/python3` с
`PYTHONPATH=/home/user/yandi` (venv, не system python3 — там отсутствуют
