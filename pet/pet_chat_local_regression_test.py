        ...
        with patch.dict("os.environ", {
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                ...
                mock_post.return_value = make_ok_response(
                    ...
                    '{"is_insult": false, "severity": 0.0, "is_apology": false, "sincerity": 0.0}'
                )
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 ...
                 patch.object(remote_backend, "generate", return_value=("Ответ от настроенного backend'а\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}", {})) as mock_remote, \
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 ...
                 patch.object(remote_backend, "generate", return_value=("ответ B\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}", {})) as mock_remote, \
                 patch.object(gw_client._session, "post") as mock_post:
                mock_post.return_value = make_ok_response("ответ A\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}")
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 patch.object(node_config, "get_model_entry", return_value=None), \
                 patch.object(gw_client._session, "post") as mock_post:
                mock_post.return_value = make_ok_response("ok\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}")
                chat_local._respond_with_character("heretic:q8", messages_in, 0.7)
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 patch.object(node_config, "get_model_entry", return_value=remote_entry("heretic:q8")), \
                 patch.object(remote_backend, "generate", return_value=("ok\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}", {})) as mock_remote:
                chat_local._respond_with_character("heretic:q8", messages_in, 0.7)
            ...
            with patch.object(node_config, "list_models", return_value={}), \
                ...
                resp = asyncio.get_event_loop().run_until_complete(chat_local.local_models())
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 ...
                 patch.object(remote_backend, "generate", return_value=("работает без Ollama\n\n###YANDI_STATE### {\"is_insult\": false, \"severity\": 0.0, \"is_apology\": false, \"sincerity\": 0.0}", {})), \
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 ...
                 patch.object(node_config, "get_model_entry", return_value=None), \
                 patch.object(gw_client._session, "post") as mock_post:
                # Marker absent entirely -> whole raw text becomes the visible reply, no crash, no event.
                mock_post.return_value = make_ok_response("Просто обычный ответ без какого-либо маркера.")
                result = chat_local._respond_with_character("heretic:q8", messages_in, 0.7)
                check("§7: marker ABSENT -> full text shown as-is, no relationship event, no crash", result == "Просто обычный ответ без какого-либо маркера." and grievance_calls == [])
            ...
            with patch.object(chat_local, "shadow_get_relationship_context", fake_shadow_ctx), \
                 ...
                 patch.object(node_config, "get_model_entry", return_value=None), \
                 patch.object(gw_client._session, "post") as mock_post:
                # Malformed JSON after the marker -> fail-open: visible text still shown, no event, no crash.
                mock_post.return_value = make_ok_response("Ответ модели.\n\n###YANDI_STATE### {not valid json at all")
                result = chat_local._respond_with_character("heretic:q8", messages_in, 0.7)
                check("§7: MALFORMED marker JSON -> fail-open, visible text still shown, no crash", result == "Ответ модели.")
